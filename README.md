...
- 全来源高清图标：自定义收藏、Chrome 书签和标签页分组共用 Retina 图标解析与受限本地缓存；支持为同一网站导入共用图标，用户图标永不被自动覆盖。
...
从仓库的 Releases 页面下载 `tab-home-v1.3.0.zip` 并解压，然后：

1. 打开 `chrome://extensions`。
2. 开启右上角“开发者模式”。
3. 点击“加载已解压的扩展程序”。
4. 选择解压后的 `tab-home-v1.3.0` 文件夹。
...
自定义收藏继续保存在原有的 `favorites` 和 `favoriteCategories` 键中，升级不会更改网址、标题、分类、自定义图标或排序。来源选择、书签文件夹折叠状态、自动图标缓存和网站级导入图标只保存在 `chrome.storage.local`；标签页分组和书签本身始终由 Chrome 管理，不会被复制为隐藏数据库。
